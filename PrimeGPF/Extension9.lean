...
This file proves the exact classification

`add p 2 = mul p 2 = exp p 2  ↔  p ∈ {2,7}`
...
/-- If every prime divisor of a positive natural is one of two fixed primes,
then the natural is a product of powers of those two primes. -/
theorem product_of_two_prime_support (a b n : ℕ)
    (ha : Nat.Prime a) (hb : Nat.Prime b) (hn : 0 < n)
  ...
  | h n ih =>
    ...
    · subst s
      ...
      have htpos : 0 < t := by
        ...
        omega
      have htn : t < n := by have := ha.two_le; nlinarith
    ...
    · subst s
      ...
      have htpos : 0 < t := by
        ...
        rw [ht0, Nat.mul_zero] at ht
        omega
      have htn : t < n := by have := hb.two_le; nlinarith
      obtain ⟨α, β, hfac⟩ := ih t htn htpos (by
        intro s hs hd
        apply h s hs
        apply dvd_trans hd
...
/-- A prime divisor of an integer with greatest prime factor five is 2, 3,
or 5. -/
...
/-- Complete classification of the triple collisions with second input 2.
The common output is necessarily 5. -/
theorem extension9_triple_second_two {p : ℕ} (hp : Nat.Prime p) :
  ...
  · rintro ⟨ham, hae⟩
    ...
    · right
      ...
      have hm5 : mul p 2 = 5 := by
        calc
          mul p 2 = add p 2 := ham.symm
          _ = 5 := h5
      have he5 : exp p 2 = 5 := by
        calc
          exp p 2 = add p 2 := hae.symm
          _ = 5 := h5

      /- First use the multiplicative kernel to force p = 1 (mod 3). -/
      have hgMul : gpf (2 * p + 1) = 5 := by
        simpa [mul, output, kernel, Nat.mul_comm] using hm5
      have hnMul : 1 < 2 * p + 1 := by have := hp.two_le; omega
      have h3mul : 3 ∣ 2 * p + 1 := by
        ...
        have hsupp : ∀ s, Nat.Prime s → s ∣ 2 * p + 1 → s = 5 := by
          ...
          · exact h5s
        obtain ⟨k, hk⟩ :=
          power_of_prime_support 5 (2 * p + 1) Nat.prime_five (by omega) hsupp
        have hpmod4 : p % 4 = 1 ∨ p % 4 = 3 := by omega
        have hNmod4 : (2 * p + 1) % 4 = 3 := by
          rcases hpmod4 with hp1 | hp3 <;>
            norm_num [Nat.add_mod, Nat.mul_mod, hp1, hp3]
        rw [hk] at hNmod4
      ...
      /- The square kernel is twice a power of five. -/
      have hgExp : gpf (p ^ 2 + 1) = 5 := by
        simpa [exp, output, kernel] using he5
      have hnExp : 1 < p ^ 2 + 1 := by
        have := hp.two_le
        nlinarith
      have hpmod4 : p % 4 = 1 ∨ p % 4 = 3 := by omega
      have hExpMod4 : (p ^ 2 + 1) % 4 = 2 := by
        rcases hpmod4 with hp1 | hp3 <;>
          norm_num [pow_two, Nat.add_mod, Nat.mul_mod, hp1, hp3]
      have hExpMod2 : (p ^ 2 + 1) % 2 = 0 := by
        have hpmod2 : p % 2 = 1 := by omega
        norm_num [pow_two, Nat.add_mod, Nat.mul_mod, hpmod2]
      ...
      have hsuppT : ∀ s, Nat.Prime s → s ∣ t → s = 5 := by
        ...
        · exact h5s
      obtain ⟨k, htk⟩ :=
        power_of_prime_support 5 t Nat.prime_five htpos hsuppT
      ...
      have hsuppAdd : ∀ s, Nat.Prime s → s ∣ p + 3 → s = 2 ∨ s = 5 := by
        ...
        · exact Or.inr h5s
      obtain ⟨α, β, hAddFac⟩ :=
        product_of_two_prime_support 2 5 (p + 3)
          Nat.prime_two Nat.prime_five (by omega) hsuppAdd
      have hAddEven : (p + 3) % 2 = 0 := by
        have hpmod2 : p % 2 = 1 := by omega
        norm_num [Nat.add_mod, hpmod2]
      have hαpos : 0 < α := by
        by_contra hα
      ...
      have hkform : k = 2 * (k / 2) := by
        have hdec := Nat.mod_add_div k 2
        omega
      have h5mod8 : 5 ^ k % 8 = 1 := by
        rw [hkform, pow_mul]
        norm_num [Nat.pow_mod]
