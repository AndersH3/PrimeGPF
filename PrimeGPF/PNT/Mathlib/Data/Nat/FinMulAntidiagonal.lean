...
private def pntPrimeFactorsPiBij (d n : ℕ) : ∀ f ∈ (n.primeFactors.pi fun _ => (univ : Finset <| Fin d)), Fin d → ℕ :=
  fun f _ i => ∏ p ∈ Finset.filter (fun p => f p.1 p.2 = i) n.primeFactors.attach, p

private theorem pntPrimeFactorsPiBij_img (d n : ℕ) (hn : Squarefree n)
  (f : (p : ℕ) → p ∈ n.primeFactors → Fin d) (hf : f ∈ pi n.primeFactors fun _ => univ) :
    Nat.pntPrimeFactorsPiBij d n f hf ∈ finMulAntidiagonal d n := by
  rw [mem_finMulAntidiagonal]
  refine ⟨?_, hn.ne_zero⟩
  unfold Nat.pntPrimeFactorsPiBij
  rw [prod_fiberwise_of_maps_to, prod_attach (f := fun x => x)]
  apply prod_primeFactors_of_squarefree hn
  apply fun _ _ => mem_univ _

private theorem pntPrimeFactorsPiBij_inj (d n : ℕ)
    (f : (p : ℕ) → p ∈ n.primeFactors → Fin d) (hf : f ∈ pi n.primeFactors fun _ => univ)
    (g : (p : ℕ) → p ∈ n.primeFactors → Fin d) (hg : g ∈ pi n.primeFactors fun _ => univ) :
    Nat.pntPrimeFactorsPiBij d n f hf = Nat.pntPrimeFactorsPiBij d n g hg → f = g := by
  contrapose!
  simp_rw [Function.ne_iff]
  intro ⟨p, hp, hfg⟩
  use f p hp
  dsimp only [Nat.pntPrimeFactorsPiBij]
...
private theorem pntPrimeFactorsPiBij_surj (d n : ℕ) (hn : Squarefree n)
    (t : Fin d → ℕ) (ht : t ∈ finMulAntidiagonal d n) : ∃ (g : _)
      (hg : g ∈ pi n.primeFactors fun _ => univ), Nat.pntPrimeFactorsPiBij d n g hg = t := by
  ...
  · rw [Nat.pntPrimeFactorsPiBij, ←prod_filter]
    congr
    ext ⟨p, hp⟩
...
theorem card_finMulAntidiagonal_pi (d n : ℕ) (hn : Squarefree n) :
  ...
  apply Finset.card_bij (Nat.pntPrimeFactorsPiBij d n) (pntPrimeFactorsPiBij_img d n hn)
    (pntPrimeFactorsPiBij_inj d n) (pntPrimeFactorsPiBij_surj d n hn)
