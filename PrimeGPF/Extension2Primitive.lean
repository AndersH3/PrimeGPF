...
/-- A natural-number specialization of the standard fact `u-1 ∣ u^g-1`.
This local lemma uses only the `Nat.ModEq` API available in the repository's
...
theorem extension2_nonprimitive_power_base
  ...
  have hu2 : 2 ≤ u := by
    ...
    have hkernel2 : 2 ≤ a * q + 1 := by
      have hmulpos : 0 < a * q := Nat.mul_pos ha.pos hq.pos
      omega
    have hupow_le : u ^ g ≤ 1 := by
      simpa using Nat.pow_le_pow_left hu1 g
    rw [← hpow] at hkernel2
    omega
