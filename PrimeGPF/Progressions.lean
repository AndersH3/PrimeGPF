...
/-! Exact arithmetic-progression reductions; the PNT-AP limit remains explicit.
New scripts, not compiler-verified. -/
namespace PrimeGPF
open Claims

/-- General affine congruence with nonzero slope and constant term. -/
...
theorem multiplicative_progression {a r : ℕ} (ha : Nat.Prime a)
  ...
  have ha0 : (a : ZMod r) ≠ 0 := by
    ...
    · exact hr.ne_one h
    · exact har h.symm
  obtain ⟨c, hc0, hcr, hc⟩ := affine_residue (b := 1) hr ha0 (by simpa using (one_ne_zero : (1 : ZMod r) ≠ 0))
...
theorem additive_progression {a r : ℕ} (hr : Nat.Prime r)
  ...
  obtain ⟨c, hc0, hcr, hc⟩ := affine_residue (a := 1) hr
    (by simpa using (one_ne_zero : (1 : ZMod r) ≠ 0)) hb
...
/-- This analytic input is still unproved in this package. -/
