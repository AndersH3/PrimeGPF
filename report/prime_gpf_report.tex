...
\usepackage{csquotes}
% Compatibility with LaTeX kernels predating the metadata conditionals.
% This document does not enable the optional PDF-management metadata layer.
...
\RedeclareSectionCommand[tocnumwidth=2.6em,beforeskip=18pt,afterskip=16pt]{chapter}
\setkomafont{section}{\normalfont\sffamily\bfseries\Large\color{DeepBlue}}
...
\vspace{4mm}
{\large Revised 14 September 2026\par}
\vfill
...
\begin{statusbox}{Formal verification snapshot as of 14 September 2026}
All \textbf{39 corrected numbered results are unconditional and compiler-verified}. The former inputs \lean{PrimeAPInput} and \lean{ZsigmondyInput} are now proved by \lean{proof_primeAP_dependency} and \lean{proof_zsigmondy_dependency}. Thus 6.1, 6.3, 8.3, 8.4, 8.5, and 9.3 no longer require external mathematical hypotheses. The combined theorem \lean{proof_all_39} proves the conjunction of all 39 corrected propositions. The full GitHub Actions build and transitive axiom audit passed on 14 September 2026 \parencite{PrimeGPFValidation}. Its dependencies are limited to \lean{propext}, \lean{Classical.choice}, and \lean{Quot.sound}. The official corrected 5.5 retains the necessary condition $r\ne2$; its originally printed false form remains separately refuted.
...
The original 15-page report was designed as a deduplicated list of the main prime-GPF results. It contained concise proofs, a computational appendix, and a bibliography, but it predated the completed Lean work for Theorem 6.2, the proof of Theorem 6.3, the two completed dependency proofs, and the correction of Theorem 5.5. The present version therefore treats the old document as a historical baseline rather than an authority whenever it conflicts with the checked formal source.
...
theorem proof_6_3_from_PNT_AP (H : PrimeAPInput) : Claims.t6_3 := by
  ...
\end{lstlisting}
proves an implication: if a term of type \lean{PrimeAPInput} is supplied, Lean checks a term of type \lean{Claims.t6_3}. This is a genuine formal proof of the implication, but does not itself prove its premise. The current library also supplies that premise and exports the unconditional theorem:
\begin{lstlisting}[language=Lean]
theorem proof_6_3 : Claims.t6_3 :=
  proof_6_3_from_PNT_AP proof_primeAP_dependency
\end{lstlisting}
The suffix ``Input'' names a proposition used as a modular interface; it does not make that proposition a postulated axiom. The distinction between a proved interface and an unresolved assumption is essential when reading the older experiment files.

\section{What ``verified'' means here}
\begin{conceptbox}{Lean elaboration, proof terms, and the kernel}
Lean translates notation, tactics, coercions, and typeclass search into a proof term in its dependent type theory. The kernel then checks that proof term against the claimed type. The Lean documentation explicitly separates the validity of a formal proof from the semantic question of whether the formal theorem statement faithfully expresses the intended mathematics \parencite{LeanValidate}. Accordingly, this report uses \emph{compiler-verified} to mean that the pinned project build accepted the declaration, not that an AI system or informal argument endorsed it.
\end{conceptbox}
...
The source is maintained in the private repository \href{https://github.com/AndersH3/PrimeGPF}{AndersH3/PrimeGPF}. The proof snapshot used here is commit \lean{f169752012e60d0decda7533c3edbc427c759d59}, validated by GitHub Actions run \lean{34798956017}; it was merged into \lean{main} as \lean{9c60cd35f329dc7380d3a17db6da23cbe7c54e9d} \parencite{PrimeGPFRepo,PrimeGPFValidation}. Lean is pinned to 4.19.0 and mathlib to \lean{c44e0c8ee63ca166450922a373c7409c5d26b00b}. Repository and workflow links require access at the time of this revision; a public archival release would improve independent reproducibility. This report update changes exposition, not the numbered Lean statements.

From a checkout of that source, run:
\begin{lstlisting}[language={}]
lake exe cache get
bash verify.sh
\end{lstlisting}
The verifier builds the full library, compiles \lean{Audit.lean}, enforces the foundational-axiom allowlist, and requires the audit of \lean{proof_all_39} before returning success (exit status 0). The targeted \lean{verify_prime_ap.sh} checks the new analytic dependency and 6.1/6.3 separately. The workflow artifact contains build and audit logs. Older module comments and archived status files describe earlier stages; the committed declarations and successful audit, rather than stale prose labels, determine current proof status.

\begin{figure}[H]
\centering
\FormalDependencyDiagram
\caption{Selected dependency paths in the formalization. Every displayed layer is proved. The two formerly external interfaces are discharged before their numbered consequences are combined in the full audit.}
...
\paragraph{Interoperability and computational scope.}
The custom implementation is a finite scan, not a fast factorizer. It is totalized by $\lean{gpf}(0)=\lean{gpf}(1)=0$; mathematical GPF specifications require $n>1$. Interoperability rests on the specification, not on unfolding the scan: \lean{gpf_eq_of_spec} proves that any candidate satisfying \lean{IsGPF n r} equals \lean{gpf n}. A future mathlib greatest-prime-factor implementation can therefore be connected by proving the same primality, divisibility, and maximality properties, with explicit treatment of 0 and 1. That bridge is possible using the current API but is not claimed to have been implemented here. Users can import the relevant \lean{PrimeGPF.Core} module for this infrastructure; importing the root \lean{PrimeGPF} also loads the analytic proof development.
...
\]
For prime $r$ and $0<c<r$, $\varphi(r)=r-1$. Davenport gives a classical treatment of primes in arithmetic progressions \parencite{Davenport2000}. The pinned mathlib's \lean{PrimesInAP} file supplies Dirichlet infinitude and the residue-class L-series facts used below. PrimeGPF now derives the required quantitative ratio by combining these with a proved Wiener--Ikehara theorem \parencite{MathlibPrimesInAP,PNTAndLean}.
\end{conceptbox}

\begin{definition}[The now-proved interface \lean{PrimeAPInput}]
...
\begin{theorem}[6.1: progression reduction and PNT-AP]\label{thm:61}
Fix primes $a,r$ with $r\ne2$.
\begin{enumerate}[label=(\alph*)]
\item If $a\ne r$, the primes $q$ for which $r\mid aq+1$ lie in exactly one nonzero residue class $q\equiv c\pmod r$; their exact finite count agrees with \lean{apCount r c x}. This count has the PNT-AP asymptotic.
\item If $(a+1)\not\equiv0\pmod r$, the primes $q$ for which $r\mid a+q+1$ likewise lie in one reduced class, with the same asymptotic conclusion.
\item If $(a+1)\equiv0\pmod r$, then for prime $q$ the additive divisibility condition is equivalent to $q=r$.
\end{enumerate}
\end{theorem}
The residue-class algebra and exact counting identities are unconditional in \lean{Progressions.lean}. The ratio limit is proved in \lean{PrimeAP.lean}; \lean{proof_6_1} combines these ingredients.

\section{How the PNT-AP interface is now proved}\label{sec:pnt-proof}
The analytic proof uses the version of \lean{WienerIkeharaTheorem'} with explicit summability and Chebyshev-bound hypotheses, adapted from \emph{PrimeNumberTheoremAnd} at commit \lean{d3cea76119684a766d2cd195b05b137442205653}. This upstream revision matches the project's Lean/mathlib pins. The retained files preserve their Apache 2.0 license and author notices. Unfinished later formulations and AP declarations are excluded: the proof does not import an admitted PNT-AP theorem. A private helper family was renamed to avoid a generated-declaration collision with mathlib; no mathematical statement was changed \parencite{PNTAndLean,PrimeGPFProvenance}.

For a reduced class $a\pmod q$, put $f(n)=\Lambda(n)\mathbf{1}_{n\equiv a\, (q)}$, where $\Lambda$ is von Mangoldt's function. Then $0\le f(n)\le\Lambda(n)$. Domination supplies the required Dirichlet-series summability for real part greater than 1 and the Chebyshev bound on partial sums. The pinned mathlib declarations \lean{vonMangoldt.continuousOn_LFunctionResidueClassAux} and \lean{vonMangoldt.eqOn_LFunctionResidueClassAux} supply the continuation and pole subtraction. Applying the proved Tauberian theorem gives
\[
 \frac1N\sum_{n<N}f(n)\longrightarrow\frac1{\varphi(q)}.
\]
The endpoint change to $n\le N$ costs at most $\log N/N$. Removing proper prime powers costs no more than the unrestricted von Mangoldt-minus-prime sum, whose normalized limit is zero by the retained proved PNT consequences. Thus, for
\[
 \theta_{q,a}(N)=\sum_{\substack{\ell\le N,\ \ell\text{ prime}\\\ell\equiv a\, (q)}}\log\ell,
 \qquad \theta_{q,a}(N)/N\longrightarrow1/\varphi(q).
\]
These steps are in \lean{PrimeAPAnalytic.lean}. The retained Kronecker experiment gives another prime-power-removal argument, but it is not the route used by the final numbered proofs.

\paragraph{From logarithmic weights to prime counts.}
The general lemma \lean{counting_limit_of_weighted} handles finite sets $S_N\subseteq\{1,\ldots,N\}$. Write $A_N=|S_N|$ and $W_N=\sum_{n\in S_N}\log n$, and suppose $W_N/N\to A>0$. The lower estimate is $W_N\le A_N\log N$. For fixed $0<\delta<1$, split at $N^\delta$ to obtain
\[
 A_N\,\delta\log N\le (N^\delta+1)\delta\log N+W_N.
\]
Since $(N^\delta+1)\log N/N\to0$, the normalized count has lower limit at least $A$ and upper limit at most $A/\delta$. Letting $\delta$ approach 1 yields $A_N\log N/N\to A$. The Lean proof uses eventual upper and lower bounds, avoiding any unproved limsup principle. Finally, \lean{PrimeAP.lean} identifies the exact inclusive count and substitutes $\varphi(r)=r-1$ to produce \lean{proof_primeAP_dependency : PrimeAPInput}.
...
\section{Zero density: the completed formal proof}
...
\begin{theorem}[6.3: zero density]\label{thm:63}
...
\end{enumerate}
All three clauses are compiler-verified by \lean{proof_6_3}, which supplies the proved \lean{PrimeAPInput} to the existing density reduction.
\end{theorem}
...
\frac{S_r((x+1)^2)}{\pi(x)}.
\]
Using the now-proved \lean{PrimeAPInput}, one fixed reduced progression already supplies an eventual lower bound $\pi(x)\gg x/\log x$, while Theorem \ref{thm:62} gives $S_r((x+1)^2)=O((\log x)^{\pi(r)})$. The quotient tends to zero.

\begin{leanbox}{The dependency is discharged}
The checked implication \lean{PrimeAPInput}\,$\Longrightarrow$\,\lean{Claims.t6_3} remains a useful modular interface. The current library also proves its premise, so the exported \lean{proof_6_3 : Claims.t6_3} is unconditional. Axiom inspection is transitive: the successful audit follows the proof through the analytic development, rather than merely inspecting this final wrapper.
...
\chapter{Exponential GPF structure and prime-exponent divisors}\label{ch:exp}
...
The formal project isolates and now proves the following specialized existence statement:
\begin{lstlisting}[language=Lean]
...
One classical route would be to use the general theorem. Apply Bang--Zsigmondy to $p^{2q}-1$. A primitive prime divisor $r$ has order $2q$, so it cannot divide $p^q-1$. Since
\[
p^{2q}-1=(p^q-1)(p^q+1),
\]
...
\section{A direct proof for prime exponents}\label{sec:prime-exponent}
The implemented proof in \lean{PrimeExponent.lean} does not formalize the full Bang--Zsigmondy theorem. It proves exactly the displayed interface using an odd cofactor, lifting the exponent, and finite-field order \parencite{PrimeExponentSource,MathlibValuation}.

Let $q=2n+1$ be an odd prime and set
\[
 C=\frac{p^q+1}{p+1}.
\]
The recursively defined \lean{oddCofactor p n} proves this quotient identity without natural-number subtraction difficulties. It is odd, and $C>q$ outside $(p,q)=(2,3)$. Suppose every prime divisor $r$ of $C$ also divides $p+1$. Reducing the cofactor at $p\equiv-1\pmod r$ gives $C\equiv q\pmod r$, so every such $r$ equals $q$. Unique factorization then gives $C=q^k$ with $k>0$. In particular $q\mid p+1$. The odd-prime lifting-the-exponent identity yields
\[
 v_q(p^q+1)=v_q(p+1)+v_q(q)=v_q(p+1)+1.
\]
Since $(p+1)C=p^q+1$, this forces $v_q(C)=1$, hence $C=q$, contradicting $C>q$. Therefore $C$ has an odd prime divisor $r$ outside $p+1$.

For this divisor, $p^q\equiv-1\pmod r$, while $p^2\not\equiv1\pmod r$: in the field modulo $r$, a square root of 1 is $1$ or $-1$, and both possibilities contradict the construction. The order divides $2q$ but divides neither $q$ nor 2; since $q$ is prime, it equals $2q$. This constructs the required primitive divisor. The declaration \lean{proof_zsigmondy_dependency} closes the interface; the general two-base, arbitrary-exponent Bang--Zsigmondy theorem is not a claimed deliverable.

\begin{theorem}[8.3: prime-exponent lower bound]\label{thm:83}
Let $p,q$ be primes with $q\ne2$ and $(p,q)\ne(2,3)$. There is a prime $r\mid p^q+1$ satisfying
\[
r\equiv1\pmod{2q}.
\]
Consequently
...
\]
\end{theorem}
The existence and order arguments are now both checked. The theorem \lean{proof_8_3} supplies the proved interface to the original order-theoretic reduction.

\begin{theorem}[8.4: near anti-projection]
...
\begin{theorem}[8.5: anchored exponential dynamics]
...
\begin{statusbox}{Completed exponential dependency}
The unconditional declarations \lean{proof_8_3}, \lean{proof_8_4}, \lean{proof_8_5}, and \lean{proof_9_3} all use \lean{proof_zsigmondy_dependency}. Their historical conditional wrappers remain available, but the numbered results no longer assume an external existence theorem.
...
\begin{theorem}[9.3: exponential dominance on the $p=2$ row]
For prime $q>3$,
\[
...
\item[\lean{Progressions.lean}] exact AP reductions, \lean{PrimeAPInput}, and the modular reduction for 6.1.
...
\item[\lean{Conditional.lean}] reusable input propositions and downstream implication wrappers.
\item[\lean{PrimeExponent.lean}] the specialized primitive-divisor proof and unconditional 8.3--8.5, 9.3.
\item[\lean{PrimeGPF/PNT/}] credited, version-compatible upstream analytic proofs.
\item[\lean{PrimeAPAnalytic.lean}] residue-class weighted asymptotics and prime-power removal.
\item[\lean{WeightedCounting.lean}] general logarithmic-weight-to-count conversion.
\item[\lean{PrimeAP.lean}] the proved AP input and unconditional 6.1, 6.3.
\item[\lean{Complete.lean}] \lean{proof_all_39}, the conjunction of all corrected numbered statements.
\end{description}
...
6.1 & AP reduction + PNT-AP & Verified & \lean{proof_6_1}, PrimeAP \\
6.2 & Polylogarithmic fibers & Verified & \lean{proof_6_2}, Counting \\
6.3 & Zero density & Verified & \lean{proof_6_3}, PrimeAP \\
...
8.3 & Prime-exponent lower bound & Verified & \lean{proof_8_3}, PrimeExponent \\
8.4 & Near anti-projection & Verified & \lean{proof_8_4}, PrimeExponent \\
8.5 & Exponential dynamics & Verified & \lean{proof_8_5}, PrimeExponent \\
9.1 & Bridge identities & Verified & \lean{proof_9_1}, Elementary \\
9.2 & Second-column rigidity & Verified & \lean{proof_9_2}, Arithmetic \\
9.3 & Exponential dominance & Verified & \lean{proof_9_3}, PrimeExponent \\
...
\item \textbf{Finite combinatorics separates cleanly from analytic input.} The exact residue-class reductions and the entire smooth/fiber-counting theory are unconditional. The PNT-AP layer is now proved and composed with them in 6.1 and 6.3.
\item \textbf{Zsigmondy is localized.} The required existence proposition is proved directly for prime exponents, making its four numbered consequences unconditional.
...
\section{Formal-library integration after the search}
The earlier search correctly distinguished Dirichlet infinitude from the quantitative AP ratio. The completed development uses more of the pinned \lean{PrimesInAP} module: its residue-class L-series continuation and pole calculation are combined with the proved Wiener--Ikehara development from a version-compatible \emph{PrimeNumberTheoremAnd} revision \parencite{MathlibPrimesInAP,PNTAndLean}. The latter is now an explicitly credited, adapted source dependency under \lean{PrimeGPF/PNT/}; unfinished upstream AP declarations are excluded. This is a concrete integration, not an inference from a project's stated objectives.

The specialized \lean{ZsigmondyInput} is proved locally using mathlib's valuation and finite-field APIs. No claim is made that a general Bang--Zsigmondy theorem has thereby been added to mathlib, or that no other Lean proof exists. The 13 September prior-art search remains a dated, non-exhaustive search record; this revision updates source integration and verification evidence rather than claiming a new exhaustive literature search.

\paragraph{Scope of the contribution.}
The work is principally a synthesis and mechanization of elementary number-theoretic statements about these three kernels, with explicit analytic dependencies and reusable formal bridges. The completed PNT-AP route and prime-exponent argument strengthen the formal coverage; they do not make classical Tauberian theory, lifting the exponent, or greatest-prime-factor magmas new mathematics. Kernel acceptance certifies the formal deductions, while novelty and correspondence with the informal formulations remain matters for mathematical review.

\chapter{Conclusions and open problems}\label{ch:conclusions}

All 39 corrected numbered results are now proved without external mathematical hypotheses. The transitive audit of \lean{proof_all_39} closes the previous six-result gap while preserving the original corrections and foundational trust policy.

\section{Further formal work beyond this inventory}
The review requested a feasibility roadmap for the missing inputs. Those tasks have now been completed at the existing Lean 4.19.0 pin: the AP route required substantial analytic source integration and a weighted-count bridge, whereas the prime-exponent route used existing valuation and order APIs without the full general Zsigmondy theorem. Remaining engineering directions include upstreaming reusable lemmas, maintaining compatibility with later mathlib releases, connecting the custom GPF specification to other implementations, and reducing reliance on copied compatibility files. Formalizing general Bang--Zsigmondy or stronger uniform AP error estimates would be separate extensions, not prerequisites for the 39 results.
...
The greatest benefit of formalization here is not merely a ``verified'' label. It is the decomposition of the argument into exact finite arithmetic, explicit exceptional cases, and two formerly external interfaces whose proofs are now supplied. That decomposition helped expose a false statement, supported independent proof development, and now yields a complete checked inventory. The contribution remains a formal synthesis with explicit scope, not a claim to new classical number theory.
...
\section{The separate Agda computational port}
The repository also contains \lean{prime-gpf-agda-verified/}, a certificate-preserving translation of the PARI/GP routines. Its types distinguish naturals greater than 1 from certified primes, with operations mapping certified primes to certified primes. The supplied verification record uses Agda 2.8.0, standard library 2.3, and the GHC backend. Its script type-checks the core, checks small definitional-equality regression proofs, and compares a compiled $5\times5$ table with the expected PARI/GP values. A heavier proof-normalization test is retained but omitted from the routine script \parencite{AgdaPort}.

This is a computational companion, not a second formalization of all 39 Lean propositions. Its test evidence is separate from the Lean axiom audit; the report revision does not claim a new Agda run.

\chapter{Lean declaration guide}\label{app:lean-guide}
\begingroup
\setlength{\parskip}{0pt}
\begin{longtblr}[caption={Lean objects and their mathematical roles}]{colspec={Q[l,wd=50mm] X[l]},colsep=2pt,hlines,rows={valign=t}}
\toprule
...
This report was produced with substantial AI assistance. AI/deep-research and browsing tools were used to locate candidate literature, compare bibliographic records, synthesize mathematical background, organize the exposition, draft and edit prose, generate XeLaTeX/TikZ/listing source, and assist with document quality control. The user supplied the mathematical project, source compendium, Lean project context, build/audit transcripts, and direction for the revision. The 14 September update also used AI assistance to compare the merged proofs with the manuscript and respond to the supplied peer review. The new informal proof explanations are expository reconstructions of committed Lean arguments, not substitutes for them.
\end{warningbox}
...
\begin{enumerate}
\item \textbf{Lean-verified claims:} status is based on the committed source and the successful GitHub Actions build/audit cited in this revision. These claims are about formal propositions actually accepted by Lean, subject to the correctness of the formal statement and imported environment.
...
\item Completed \lean{PrimeAPInput} and made 6.1 and 6.3 unconditional, including the existing pair-count and limit architecture.
\item Proved the specialized \lean{ZsigmondyInput}, making 8.3--8.5 and 9.3 unconditional; added the combined \lean{proof_all_39} audit.
\end{itemize}
...
\begin{itemize}
\item Reproduce the cited commit and audit when preparing a publication release, and review informal/formal statement correspondence.
...
\item Check visual and text-extraction quality of the consistent symbols $\boxplus$, $\boxtimes$, and $\gexp$ in the intended publication workflow.
\end{itemize}
