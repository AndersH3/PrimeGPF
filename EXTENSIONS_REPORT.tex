...
\midrule
1 & proved below & finite reduction, exact support dimension, and general polylogarithmic bound compiler-verified; sharp coefficient (1) verified when $|T|=1$, including $a=r=5$; general $|T|>1$ sharp coefficient remains \\
2 & proved below & empty $a=r$ case, support restriction, primitive/nonprimitive decomposition, exceptional-family bounds, exact support dimension, and general polylogarithmic bound compiler-verified; the $1/\zeta(d)$ coefficient (2) remains \\
3 & proved below & exact classification (3), coprimality, residue restriction, exact support $\{3,5\}$, and quadratic polylogarithmic bound compiler-verified; sharp constant (4) remains \\
4 & proved below & exact classification (5), coprimality, residue restriction, exact support $\{2,5\}$, and quadratic polylogarithmic bound compiler-verified; sharp constant (6) remains \\
5 & proved below & exponential escape (7), finite visit count, and the literal real-$\log_2$ form of (8) compiler-verified \\
6 & proved below & finite image bound (9), PNT scaling and limsup bound (10), and deep-image classification (11) compiler-verified \\
7 & proved below & finite restricted-support bound, density zero, prime-relative rate (13), and sharp coefficient (12) when $d_a=1$ compiler-verified; general $d_a>1$ sharp coefficient remains \\
8 & proved below & inequalities (14)--(16) and the prime-polynomial no-triple-collision corollary compiler-verified \\
9 & proved below & complete $q=2$ classification (17) and common output $5$ compiler-verified \\
10 & proved below & both algebraic and dynamical separation compiler-verified \\
\bottomrule
\end{tabularx}
\end{center}

The extension development is split into dedicated modules under \path{PrimeGPF/}; verified leaf modules are imported by the top-level \path{PrimeGPF.lean}. The general weighted-simplex asymptotic (*) and the Mobius-inversion refinements are the principal remaining analytic infrastructure.
...
The one-dimensional specialization of (*) needed when $|T|=1$ is formalized in Lean. The general multidimensional weighted-simplex estimate remains to be formalized.
...
\]
The general finite reduction and logarithm-saving exponent are Lean-certified, and the sharp limsup coefficient in (1) is Lean-certified whenever $|T|=1$, in particular for $a=r=5$.
...
Lean currently formalizes the support restriction, the primitive/nonprimitive dichotomy, the two exceptional perfect-power families, their logarithmic-size bound, the exact support dimension, and the resulting non-sharp polylogarithmic fiber bound. The Mobius-inversion step producing $1/\zeta(d)$ remains.
...
The exact classification, coprimality condition, congruence restriction, and exact exponent support $\{3,5\}$ are Lean-certified. The parity-restricted Mobius count giving (4) remains.
...
The exact classification, coprimality and parity conditions, congruence restriction, and exact exponent support $\{2,5\}$ are Lean-certified. The parity-restricted Mobius count giving (6) remains.
...
The Lean development proves (7), the corresponding finite cardinality bound, and (8) in the literal natural-floor/real-base-two-logarithm form displayed above.
...
The Lean development proves (9), the PNT fixed-scale ratio and limsup estimate (10), and the exact all-depth classification (11).
...
Lean proves the finite restricted-support reduction, a degree-$d_a$ polylogarithmic upper bound, density zero, and the prime-relative Big-O rate (13). The sharp coefficient in (12) is additionally formalized when $d_a=1$; the general $d_a>1$ coefficient remains tied to (*).
...
All three inequalities and this prime-polynomial corollary are Lean-certified.
...
The equivalence (17) and the common-output-$5$ conclusion are Lean-certified.
...
The eventual-periodicity result and both intertwining obstructions are Lean-certified in \path{PrimeGPF/Extension10Dynamics.lean}.

\section{Remaining directions}

The results above do \emph{not} establish asymptotic equivalences for the actual prime fibers, nor their infinitude, nor a global classification of triple collisions with odd second input. The principal remaining formalization task for extensions 1--4 and 7 is the general weighted-simplex counting estimate (*); the $\zeta$-refinements in extensions 2--4 additionally require Mobius inversion, including parity-restricted primitive-vector counts for (4) and (6). The one-dimensional sharp cases of (1) and (12), the full PNT passage in (10), and the prime-relative rate (13) are already formalized.
